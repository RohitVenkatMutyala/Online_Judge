...
function CreateSession() {
    ...
    const [step, setStep] = useState(0); // 0: initial screen, 1: description, 2: settings, 3: emails
    ...
    // --- UPDATED: This function now uses data from state instead of prompts ---
    const handleCreateSession = async () => {
        ...
        if (accessType === 'private') {
            invitedEmails = emailsInput.split(',').map(email => email.trim()).filter(Boolean);
    ...
    // --- Helper functions to render each step of the form ---
    const renderStep = () => {
        switch (step) {
            ...
            case 3: // Emails Step
                return (
                     <div className="card-body p-5">
                        ...
                        <p className="text-muted mb-4">Enter the email addresses of the people you want to invite, separated by commas.</p>
                        <textarea
                            className="form-control"
                            rows="4"
